    ...
    if (allPassages[index][0] == 'sub'){
        ...
        Array.from(targetTable.querySelectorAll("td")).forEach((cell) => {
            ...
            cell.innerHTML = originalContent.replace(regex, (match, type, content) => {
                ...
                if (type === "a") { /// 표는 정신이 너무 없어서 question-number를 일단 지움.
                    ...
                    questionHTML = `<div class="input-container" id="${inputId}" data-answer="${content}" data-type="a">
                                        <span class="question-number" style="display: none;">${questionNumber}</span>
                                        <input type="text" placeholder="" data-type="userinput" size="${inputSize}"/>
                                    </div>`;
                    globalQuestionCounter++;
        ...
        let replacedPassage = passageWithLineBreaks.replace(regex, (match, type, content) => {
            ...
            if (type === 'a') { //초성형 문제
                ...
                if (qndisplay == 0){
                    const inputSize = Math.max(answer.length, 2)*1.7;
                    ...
                    questionHTML = `<span class="hint-container" id="${inputId}" data-answer="${answer}" data-type="a">
                        ...
                        const inputSize = Math.max(answer.length, 2)*1.4;
                ...
                const inputSize = Math.max(hint.length, 3)*1.7;
                ...
                if (qndisplay == 0){
                    ...
                    questionHTML = `<span class="input-container" id="${inputId}" data-answer="${answer}" data-type="a">
                                        <span class="question-number">${questionNumber}</span>
                                        <input type="text" placeholder="" data-type="userinput" size="${inputSize}"/>
                                    </span>`;
                }else{
                ...
                questionHTML = `<span class="input-container" id="${inputId}" data-answer="${content}" data-type="a" style="margin-left: 0px;">
                                    <span class="question-number">${questionNumber}</span>
                                    <input type="text" placeholder="" data-type="userinput" size="${inputSize}"/>
                                </span>`;
            } else if (type === 'd') {
            ...
            if (allPassages[index][0] === 'C') {
                ...
                passageContext.style.width = "90%";
                        if (ENum_doubled === 'L') {
                            ...
                            if (container) {
                            const nummarks = container.querySelectorAll('.nummark_small');
                            if (nummarks.length > 0) {
                                ...
                                const nummarksAll = document.querySelectorAll('.nummark_small');
